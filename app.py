...
def run_open_ended_business_analysis(question: str) -> dict:
    """
    Run a deterministic 5-query business diagnostic.

    Important:
    - No ambiguous revenue metric is used.
    - Each query is validated before reading results.
    - The LLM is used only to synthesize already-executed SQL evidence,
      not to invent or generate tool calls.
    """
    ...
    diagnostic_queries = [
        ...
        # 3. Delivery performance for valid delivered orders
        """
        SELECT
        ...
        # 4. Payment type mix (counts only; no ambiguous revenue)
        """
        SELECT
            payment_type,
            COUNT(*) AS order_count,
    ...
    if not sql_queries:
        return {
            "answer": (
                "Không thể hoàn tất business diagnostic vì không có "
                "SQL query nào vượt qua validation."
            ),
            "sql": [],
            "data_tables": [],
            "audit": [],
            ...
            "limitations": [
                "Business diagnostic could not execute validated SQL."
            ],
    ...
    _, structured_llm = build_agent()

    synthesis_prompt = f"""
You are preparing a concise executive business diagnostic from ALREADY EXECUTED SQL evidence.
...
ANSWER
- Give a short executive summary of what the data suggests.
- Use only evidence contained in SQL EVIDENCE.
- Never invent numbers, currency, causality, profitability, demand,
  customer preference, or market size.

BASIC INSIGHTS
- Return 2-4 concise basic insights.
- Each must cite a concrete SQL-supported pattern or number.

PARADOXICAL INSIGHTS
- Return 1-2 evidence-based paradoxical insights:
  a surprising contrast, tension, exception, or counter-intuitive pattern.
- Do not manufacture a paradox.
- If none is defensible, return exactly:
  "No defensible paradoxical pattern can be established from the current SQL evidence."

STRATEGY
Return:
- short_term: 2-3 immediate operational actions
- medium_term: 2-3 process / analysis / resource-allocation actions
- long_term: 2-3 structural strategic actions

Every strategy must explicitly follow from the evidence.
Where evidence is insufficient, recommend what to measure next rather than
pretending the data proves more than it does.

LIMITATIONS
- Mention that product/category evidence refers to retained product rows
  in the transformed dataset.
- Do not use basket-size, repeat-customer, or original payment-count claims.

CHART
- chart.type must be "none" for this broad diagnostic unless the user explicitly asked for a chart.
"""

    try:
        presentation = structured_llm.invoke(synthesis_prompt)
    except Exception:
        # Safe deterministic fallback: still provide a usable report shell.
        presentation = Presentation(
            answer=(
                "Đã chạy business diagnostic trên 5 chiều: fulfillment, "
                "geography, delivery, payment mix và retained product categories. "
                "Xem các bảng dữ liệu và SQL audit bên dưới."
            ),
            basic_insights=[
                "Các bảng SQL bên dưới chứa bằng chứng trực tiếp cho business diagnostic.",
            ],
            paradoxical_insights=[
                "No defensible paradoxical pattern can be established from the current SQL evidence.",
            ],
            strategy=StrategyPlan(
                short_term=[
                    "Ưu tiên xử lý các tín hiệu vận hành nổi bật nhất trong SQL evidence.",
                ],
                medium_term=[
                    "Bổ sung phân tích theo thời gian và theo phân khúc trước khi tái phân bổ nguồn lực.",
                ],
                long_term=[
                    "Xây dựng bộ KPI nhất quán để theo dõi fulfillment, geography, payment mix và category concentration.",
                ],
            ),
            limitations=[
                "Category evidence refers to retained product rows in the transformed dataset.",
            ],
